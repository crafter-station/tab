...
Tab is local-first: Automatic Suggestions are Local Suggestions and never silently fall back to cloud inference. Double-tapping Option invokes an explicit cloud-backed action for harder writing moments: Deep Complete at a reliable caret or, as added by ADR-0046, Rewrite for a reliable non-empty selection. This supersedes ADR-0026, ADR-0027, and ADR-0029 and partially supersedes the metering scope in ADR-0025 and ADR-0028.
...
Local usage counts only Accepted Words inserted through deliberate Acceptance. Generated, shown, ignored, dismissed, stale, empty, and failed Local Suggestions do not consume the daily allowance. Deep Complete allowance usage counts once only when an explicit Deep Complete or Rewrite request returns a Suggestion; internal retries, empty responses, and failures do not count. A Rewrite target that becomes stale after dispatch is never replaced, but a replacement already returned by the API has consumed allowance. Exhausting either allowance preserves the rest of the product and presents an upgrade path rather than making Tab appear broken.
