...
The current candidate is directly observed signed in, but macOS currently exposes `com.apple.loginwindow` as the active application and will not make automated application windows frontmost. Real-app focus and selection therefore cannot reach the candidate helper. Option+Tab, overlay click, exact replacement, clipboard restoration, rich-text replacement, and stale-generation behavior remain unclaimed as real-app passes for TextEdit, Notes, Mail, Slack, Discord, Messages, VS Code, Obsidian, or Chrome. A human must unlock the graphical macOS session before automation can collect the remaining evidence; until then, the observed lock-state target retains the required no-request/no-replacement result.
