...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TextEdit | 1.20, installed | Reliable native selection target | Production helper emitted exact selected text and range before and after session unlock | Not observed end to end | Not observed end to end | Not observed end to end | Exact 2,000-character selection emitted without truncation; recovered two-line fixture emitted exact range `0:48` | Sentinel remained unchanged during a non-Acceptance attempt; restoration not observed | First probe switched to another app and safely emitted an unreliable null-range snapshot; stable and recovered retries were reliable | Rich-text document selection was exercised, but generated plain-text replacement was not; synthetic double-Option attempts were inconclusive and are not product-failure evidence |
...
The current candidate is directly observed signed in, and the unlocked session now permits reliable TextEdit focus and selection. However, automated modifier events did not provide a trustworthy Rewrite trigger while unrelated active input was present, so no end-to-end Acceptance claim was made. Option+Tab, overlay click, exact replacement, clipboard restoration, rich-text replacement, and stale-generation behavior remain unclaimed as real-app passes for TextEdit, Notes, Mail, Slack, Discord, Messages, VS Code, Obsidian, or a supported browser editor. Continue only in a clearly idle validation window; the earlier loginwindow and Chrome for Testing observations retain their required no-request/no-replacement results.
