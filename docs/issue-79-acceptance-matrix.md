...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TextEdit | 1.20, installed | Reliable native selection target | Production helper emitted exact selected text and range before and after session unlock; a physical double-Option produced no overlay because the helper's 400 ms threshold was shorter than the system's 500 ms double-click interval | Not observed end to end | Not observed end to end | Not observed end to end | Exact 2,000-character selection emitted without truncation; recovered two-line fixture emitted exact range `0:48` | Sentinel remained unchanged during a non-Acceptance attempt; restoration not observed | First probe switched to another app and safely emitted an unreliable null-range snapshot; stable and recovered retries were reliable | Rich-text document selection was exercised, but generated plain-text replacement was not; trigger remediation and the remaining journey must be observed on the rebuilt candidate |
