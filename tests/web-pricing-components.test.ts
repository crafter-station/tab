...
describe("pricing plan composition", () => {
  it("preserves plan content and native monthly checkout forms", () => {
    ...
    expect(body).toInclude("Everyday suggestions with occasional deeper help.");
    expect(body).toInclude("Unlimited suggestions, more Deep Suggestions, and automatic memory.");
    expect(body).toInclude("Everything in Pro, with more Deep Suggestions.");
  ...
  it("preserves signed-in and signed-out action destinations", () => {
    ...
    expect(marketingSource).toInclude("Sign in, then continue to checkout.");
    expect(marketingSource).toInclude("Continue to checkout or manage your current plan.");
    expect(marketingSource).toInclude("Change or cancel your plan");
  });
});
